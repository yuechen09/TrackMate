...
public class OnRequestUpdater extends Thread
{
	 ...
	 */
	public OnRequestUpdater( final Refreshable refreshable, final JComponent componentToListen )
	{
		super( "OnRequestUpdater for " + refreshable.toString() );
		this.refreshable = refreshable;

		/*
		 * Add an auto listener that will unlink thhe refreshable when the
		 * component is closed. But we only add it when the component is added.
		 */
		componentToListen.addAncestorListener( new AncestorListener()
		{
			...
			{
				SwingUtilities.getWindowAncestor( componentToListen ).addWindowListener( new WindowAdapter()
				{
					@Override
					public void windowClosing( final java.awt.event.WindowEvent e )
					{
						...
						System.out.println( "Updater joined for " + componentToListen ); // DEBUG
					};
				} );
				System.out.println( "Listener added to " + componentToListen ); // DEBUG
			}
		} );
